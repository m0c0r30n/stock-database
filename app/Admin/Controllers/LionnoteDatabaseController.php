...
class LionnoteDatabaseController extends Controller
{
    ...
    protected function grid()
    {
        ...
        $grid->id('ID')->modal('情報', function () {
            ...
            return new Table(['ID', '日にち', '銘柄コード', '銘柄名'], $data);
        })->sortable();
        ...
        $grid->stock_name('銘柄名')->setAttributes(['style' => 'min-width:100px;'])->editable();
     ...
     */
     ...
     {
       $show = new Show(Lionnote::findOrFail($id));
    ...
    {
        $form = new Form(new Lionnote);

        $form->display('id', 'ID');
        $form->date('date', '日付');
        $form->number('stock_number', '銘柄コード');
        $form->text('stock_name', '銘柄名');
        $form->textarea('description', '説明文');
        $form->number('stock_ranking', '銘柄順位');
        $form->text('dekidaka', '出来高');
        $form->text('over_under', 'O/U比');
